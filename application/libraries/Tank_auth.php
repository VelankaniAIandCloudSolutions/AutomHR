...
class Tank_auth
{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->error = array('inactive' => 'Your Account has been Inactivated');
	...
	{
		 return $this->ci->db->select('department_id')->where('id',$id)->get('users')->row()->department_id;
	}
	/**
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $fullname, $company, $role, $phone, $email_activation, $company_name, $individual, $designation_id,$department_id,$is_teamlead,$ro)
	{
	...
	*/
	...
	// function admin_create_user($username, $email, $password, $fullname, $company, $role, $phone, $doj, $email_activation,$designation_id,$department_id,$is_teamlead,$teamlead_id)
	function admin_create_user($username, $email, $password, $fullname, $company, $city, $state, $address, $pincode, $role, $user_type, $phone, $doj, $email_activation,$designation_id,$department_id,$is_teamlead,$ro,$branch_id)
	{
		...
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			if(!empty($_POST['location'])){
				$profile['location1']	= $_POST['location'];
			}
			if(!empty($_POST['type_change'])){
				$profile['type_change']	= $_POST['type_change'];
			}
			if(!empty($_POST['reporting_to2'])){
				$profile['reporting_to2']	= $_POST['reporting_to2'];
			}
			if(!empty($_POST['personal_email'])){
				$profile['personal_email']	= $_POST['personal_email'];
			}
			if(!empty($_POST['emp_gjd'])){
				$profile['emp_gjd']	= date('Y-m-d',strtotime($_POST['emp_gjd']));
			}
			if(!empty($_POST['dob'])){
				$profile['dob']	= date('Y-m-d',strtotime($_POST['dob']));
			}
			if(!empty($_POST['seat_location'])){
				$profile['seat_location']	= $_POST['seat_location'];
			}
			if(!empty($_POST['division'])){
				$profile['division']	= $_POST['division'];
			}
			if(!empty($_POST['business_unit'])){
				$profile['business_unit']	= $_POST['business_unit'];
			}
			if(!empty($_POST['cost_center'])){
				$profile['cost_center']	=$_POST['cost_center'];
			}
			if(!empty($_POST['dynamic_field'])){
				$profile['dynamic_field']	=$_POST['dynamic_field'];
			}
			if(!empty($_POST['employee_level'])){
				$profile['employee_level']	=$_POST['employee_level'];
			}
			if(!empty($_POST['employee_id'])){
				if($_POST['branch_prefix'] != '') {
					$profile['emp_code']	= $_POST['branch_prefix'].''.$_POST['employee_id'];
				} else {
					$profile['emp_code']	= $_POST['employee_id'];
				}
				$profile['employee_id']	= $_POST['employee_id'];
			}
			...
			if (!is_null($res = $this->ci->users->create_user($data, $profile, !$email_activation))) {
				$data['user_id'] = $res['user_id'];
								...
								if($_POST['user_type']=='44')
								{
									if(isset($_POST['branch']) && !empty($_POST['branch']))
									{
										foreach($_POST['branch'] as $branch_det)
										{
											$this->ci->db->insert('dgt_assigned_entities',array('user_id'=>$data['user_id'],'branch_id'=>$branch_det));
										}
									}
								}
	...
	function admin_create_branch_admin($username, $email, $password, $fullname, $company, $city, $state, $address, $pincode, $role, $user_type, $phone, $doj, $email_activation,$designation_id,$department_id,$is_teamlead,$ro,$branch_id)
	{
		// echo $ro; exit;
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');
			...
			if (!is_null($res = $this->ci->users->create_user($data, $profile, !$email_activation))) {
				...
				$vals = array('company_logo', 'website_name', 'logo_or_icon', 'site_favicon', 'site_appleicon','system_font','sidebar_theme','theme_color','top_bar_color','login_title');
				$this->ci->db->where_in('config_key',$vals);
				$res=$this->ci->db->where('branch_id',0)->get('config')->result_array();
				if(isset($res) && !empty($res))
				{
					foreach($res as $r)
					{
						$this->ci->db->insert('dgt_config',array('branch_id'=>$profile['branch_id'],'config_key'=>$r['config_key'],'value'=>$r['value']));
					}
				}
	...
	function user_reset_password($password){
        $hasher = new PasswordHash(
                    $this->ci->config->item('phpass_hash_strength', 'tank_auth'),
                    $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
            $hashed_password = $hasher->HashPassword($password);
