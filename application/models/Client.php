...
class Client extends CI_Model
{
	...
	*/
	...
	{
		$ci =& get_instance();
		if($ci->session->userdata('user_id') != 1 && $ci->session->userdata('user_type_name') =='company_admin'){
			$branches  = self::$db->select('dgt_branches.*')->from('dgt_branches')->join('dgt_assigned_entities','dgt_branches.branch_id = dgt_assigned_entities.branch_id')->where('dgt_branches.branch_status','0')->where('dgt_assigned_entities.user_id',$ci->session->userdata('user_id'))->get()->result_array();
			$branches = array_column($branches,'branch_id');
			return self::$db->where(array('is_lead' => 0))->where_in('branch_id',$branches)->order_by('company_name','ASC')->get('companies')->result();
		 }
		 else{
			$branches = self::$db->select('branch_id,branch_status,branch_name')->from('branches') ->where('branch_status','0')->get()->result_array();
			$branches = array_column($branches,'branch_id');
			if($ci->session->userdata('user_id') ==1){
				return self::$db->where(array('is_lead' => 0))->order_by('company_name','ASC')->get('companies')->result();
			}
			else{
				return self::$db->where(array('is_lead' => 0,'branch_id'=>$ci->session->userdata('user_id')))->order_by('company_name','ASC')->get('companies')->result();
			}
		 }
	...
	{
		self::$db->order_by('depthidden','ASC')->get('departments')->result();
