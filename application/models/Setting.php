...
class Setting extends CI_Model
{
    ...
    static function update_template($group,$data){
		$ci =& get_instance();
        return self::$db->where('branch_id',$ci->session->userdata('branch_id'))->where('email_group',$group)->update('email_templates',$data);
    }
    ...
    function add_translation($language, $files)
    {
        ...
        function GetReporterById($r_id)
        {
            return $this->db->get_where('dgt_lead_reporter',array('reporter_id'=>$r_id))->row_array();
        }
