...
class All_departments extends MX_Controller
{
    ...
    public function check_depart_name()
    {
        $depart_name = $this->input->post('department_name');
        $depart_check = $this->db->get_where('departments',array('deptname'=>$depart_name))->num_rows();
        if($depart_check > 0){
            echo "false"; exit;
        }else{
            echo "true"; exit;
