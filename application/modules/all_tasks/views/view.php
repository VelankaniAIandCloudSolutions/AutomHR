 ...
 -->
						...
						<div class="col-sm-4">
							<a href="<?php echo base_url(); ?>projects" class="btn back-btn bk-alt" ><i class="fa fa-chevron-left"></i>Back</a>			
										...
										<div class="pull-right board-view-header">
															...
															<ul id="task-list">
																		...
																		foreach($all_tasks as $tasks){
																			if($tasks['task_progress'] == 100)
																			{
																	         ...
																	         $assignrds_name=$assign_member['fullname'];
																	         			 $task_board= array();
																	         $task_board = $this->db->get_where('task_board',array('task_board_id'=>$tasks['status']))->row_array(); 
																	          // print_r($task_board);exit;
																	...
																	<li class="task <?php echo $cls; ?>">
																		...
																		<span class="task-action-btn task-btn-right">
																			<span class="tsk-status label label-<?php echo $class;?>"><?php echo $tasks['priority']?></span>
																			<!-- <?php if($cls == ''){ ?>
																				<span class="tsk-status label label-danger">Pending</span>
																			<?php }else{ ?>
																				<span class="tsk-status label label-success">Complete</span>
																			<?php } ?> -->
																			<span class="tsk-status label label-<?php echo @$task_board['task_board_class']?>"><?php echo  @$task_board['task_board_name'];?></span>
																			<span class="tsk-due-date"><?php echo date('M d',strtotime($tasks['due_date'])); ?></span>
																			<span class ='remove_hidden_user' id="assign_tasks<?php echo $tasks['t_id']; ?>">
																					<div class="pro-teams">	

																<div class="pro-team-members">
																	<div class="avatar-group" style="display: inline;">
																	...
																	<?php }?>
																		<div class="avatar">
																			<!-- <a class="pull-right btn btn-primary btn-xs" data-toggle="ajaxModal" href="<?=base_url()?>projects/team/<?=$p->project_id?>"> -->
																			<a class="avatar-title rounded-circle border border-white" data-toggle="ajaxModal" href="<?=base_url()?>all_tasks/assign_user/<?=$project_id['project_id']?>/<?=$tasks['t_id']?>/Assign"><i class="fa fa-plus"></i></a>
																		</div>
																	</div>
																</div>
															</div>
             	...
             	?>
						...
						<div class="col-xs-5 message-view task-chat-view" id="task_window">
							<div class="chat-window">
															...
															<div class="pro-teams">	
																...
																<div class="pro-team-members">
																	...
																	<div class="avatar-group">
																		  ...
																		  //print_r(count($all_members));exit;
																		  $first_user=0;
								...
								<div class="chat-footer">
									<div class="message-bar">
										<form method="post" id="post_comments"   enctype="multipart/form-data" action="#" class="message-inner">
											<input type="file" id="file_upload"  name="projectfiles" style="display:none"/>
											<a class="link attach-icon" href="#" id="OpenImgUpload" data-target="#drag_files"><img src="<?php echo base_url();?>images/attachment.png" alt=""></a>
											<div class="message-area"><div class="input-group">
												<input type="hidden" id='project_upl' name="project" value="<?php echo $project_id['project_id'];?>">
												<input type="hidden" name="task" id='task_k' value="<?php echo $tasks_id;?>">
												<textarea class="form-control" id="comments" name="description" placeholder="Description"></textarea>
												<span class="input-group-btn">
													<button class="btn btn-custom" id='_comment_upload' type="button"><i class="fa fa-send"></i></button>
												</span>
												</div>
											</div>
										</form>
