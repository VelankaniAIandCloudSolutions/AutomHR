...
// exit;
    ...
    public function punch_in_post()
    {
        if($this->is_valid == TRUE)   {
            ...
            $file_name = time().underscore($_FILES['user_profile_pic']['name']);
            $uploadfile = $uploaddir.$file_name;
            if (move_uploaded_file($_FILES['user_profile_pic']['tmp_name'], $uploadfile)) {
                $result = base_url().$uploadfile;
                $inputs['user_profile_pic'] = $file_name;
            }
            ...
            $response['data'] = $data;
                 ...
                 $strtotime = strtotime(date('Y-m-d H:i'));
                  ...
                  $a_cin     = date('H:i',$strtotime); 
                   ...
                   $record = $this->user->get_attendance_data($this->token,$inputs);
    ...
    public function offline_punch_in_post()
    {
        if($this->is_valid == TRUE)   {
            ...
            $response['data'] = $data;
                 ...
                 $strtotime = strtotime($inputs['punch_date']);
                  ...
                  $record = $this->user->get_attendance_data_offline($this->token,$inputs);
        ...
        if($this->is_valid == TRUE)   {
            ...
            $response['data'] = $data;
                  ...
                  $a_cout     = date('H:i',$strtotime); 
                   ...
                   $record = $this->user->get_punchout_attendance_data($this->token,$inputs);
    ...
    public function offline_punch_out_post()
    {
        if($this->is_valid == TRUE)   {
            $inputs = $this->input->post();
            $uploaddir = 'assets/uploads/';
            ...
            $response['data'] = $data;
                  ...
                  $a_cout     = date('H:i',$strtotime); 
                   ...
                   $record = $this->user->get_punchout_attendance_data_offline($this->token,$inputs);
   ...
   public function all_branches_post()
   {
        $all_branches = $this->user->getAllBranches();

        if(!empty($all_branches)){
            $response['status_code'] = 1;
            $response['message'] = 'SUCCESS';
            $response['data'] = $all_branches;
        }else{
            $response['status_code'] = 1;
            $response['message'] = 'SUCCESS';
