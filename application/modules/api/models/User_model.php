...
class User_model extends CI_Model
{
	...
	public function is_valid_token($token)
	{
	 ...
	 public function is_valid_login($input)
	 {
		...
		if($hasher->CheckPassword($password, $check_password)){
		 	...
		 	$this->db->select('U.id,AD.fullname,U.unique_code,U.email,U.role_id,AD.branch_id');
			...
			if(!empty($record)){
				$role    = $record['role_id'];
			 	if($role == 1 || $role == 4){
					...
					$account_details['employee_id'] = $inputs['employee_id'];
					$account_details['emp_code'] = $inputs['emp_code'];
			 		$account_details['fullname'] = $inputs['fullname'];
			 		$account_details['doj'] = $inputs['emp_doj'];
			 		$account_details['phone'] = $inputs['phone'];
			 		$account_details['branch_id'] = $inputs['branch_id'];
		...
		$records = array();
			if(!empty($record)){
                 ...
                 if(isset($inputs['due_date']) && !empty($inputs['due_date']))
                {
					...
					$projects['branch_id']=$inputs['branch_id'];
	  ...
	  public function create_invoice($inputs,$token)
	 {
		...
		$records = array();
			if(!empty($record)){
                ...
                $invoices['branch_id']=$this->input->post('branch_id');
	...
	public function create_estimate($inputs,$token)
	 {
		...
		$records = array();
			if(!empty($record)){
                ...
                $estimates['branch_id']=$this->input->post('branch_id');
   ...
   function crypto_rand_secure($min, $max) {
	...
	public function get_attendance_data($token,$inputs)
	{
                    ...
                    $a_day -=1;
                     ...
                     if(!empty($record_day[$a_day]) && !empty($month_days_in_out_record[$a_day])){
                      ...
                      if($record_day[$a_day]['punch_in'] ==''){
                        ...
                        $record_day[$a_day]['punch_in_latitude'] = $inputs['latitude'];
                        $record_day[$a_day]['punch_in_longitude'] = $inputs['longitude'];
                        $record_day[$a_day]['punch_in_user_profile_pic'] = $inputs['user_profile_pic'];
                      }
                      
                      if($total_records == 1 && empty($current_day['punch_out'])){
                        
                        $current_days = array('day'=>1,'punch_in'=>$a_cin,'punch_out'=>'','punch_in_latitude'=>$inputs['latitude'],'punch_in_longitude'=>$inputs['longitude'],'punch_in_user_profile_pic'=>$inputs['user_profile_pic']);
                        $month_days_in_out_record[$a_day][0] = $current_days;
                      }else{
                        
                        if(!empty($current_day['punch_in']) && !empty($current_day['punch_out']))
                        {
                          $current_days[$total_records] =array('day'=>1,'punch_in'=>$a_cin,'punch_out'=>'','punch_in_latitude'=>$inputs['latitude'],'punch_in_longitude'=>$inputs['longitude'],'punch_in_user_profile_pic'=>$inputs['user_profile_pic']);
	...
	public function get_attendance_data_offline($token,$inputs)
	{
		$records =  $this->get_role_and_userid($token);
			...
			if(!empty($records)){
				...
				$user_id    = $records['user_id'];
				$strtotime = strtotime($inputs['punch_date']);
				  $a_year    = date('Y',$strtotime);
			      $a_month   = date('m',$strtotime);
			      $a_day     = date('d',$strtotime);
			      $a_cin     = date('H:i',$strtotime);
	...
	public function get_punchout_attendance_data($token,$inputs)
	{
		$records =  $this->get_role_and_userid($token);
				...
				$strtotime = strtotime(date('Y-m-d H:i'));
			      ...
			      if(!empty($record['month_days'])){
			        ...
			        if(!empty($record_day[$a_day])){
			            ...
			            $record_day[$a_day]['day'] = 1;
			            $record_day[$a_day]['punch_out_latitude'] = $inputs['latitude'];
                        $record_day[$a_day]['punch_out_longitude'] = $inputs['longitude'];
                        $record_day[$a_day]['punch_out_user_profile_pic'] = $inputs['user_profile_pic'];
			        }
			        if($total_records == 1 && empty($current_day['punch_out'])){
			           
			            $month_days_in_out_record[$a_day][0]['punch_out'] = $a_cout;
			            $month_days_in_out_record[$a_day][0]['punch_out_latitude'] = $inputs['latitude'];
			            $month_days_in_out_record[$a_day][0]['punch_out_longitude'] = $inputs['longitude'];
			            $month_days_in_out_record[$a_day][0]['punch_out_user_profile_pic'] = $inputs['user_profile_pic'];
			          }else{
			            ...
			            if(!empty($current_day['punch_in']) && empty($current_day['punch_out']))
			            {
			               ...
			               $current_days[$total_records-1]['punch_out_latitude'] = $inputs['latitude'];
			               $current_days[$total_records-1]['punch_out_longitude'] = $inputs['longitude'];
			               $current_days[$total_records-1]['punch_out_user_profile_pic'] = $inputs['user_profile_pic'];
			               $month_days_in_out_record[$a_day] = $current_days;
			            } 
			          }
	...
	public function get_punchout_attendance_data_offline($token,$inputs)
	{
		$records =  $this->get_role_and_userid($token);
	...
	{
		return $this->db->get_where('branches',array('branch_status'=>'0'))->result_array();
	}
