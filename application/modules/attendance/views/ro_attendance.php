        ...
        if(!empty($user_schedule)){
          ...
          $missing_work  =  $missing_work;
                    ...
                    if ($status == '0') {
						...
						$user_ar_where = array('user_id'=>$user_id,'a_day'=>$day,'a_month'=>$attendance_month,'a_year'=>$attendance_year);
						$user_ar = $this->db->get_where('dgt_attendance_details_ar',$user_ar_where)->result_array(); 
						if(count($user_ar) > 0)
						{
							if($user_ar[0]['ro_status']==2)
							{
								$ic='<i style="color:red;" class="fa fa-bell" aria-hidden="true"></i>';
							}
							else
							{
								$ic='<i class="fa fa-bell" aria-hidden="true"></i>';
							}
							
							$ar_func='update_attendance_ar_ro';
						}
						else
						{
							$ic='-';
							$ar_func='update_attendance';
						}
                        $attendance_body .= '<a href="'.base_url() . 'attendance/'.$ar_func.'/'.$user_id.'/'.$day.'/'.$attendance_month.'/'.$attendance_year.'" data-toggle="ajaxModal" >'.$ic.'</a>';
	                    }else if(($punch_in == ''  || $punch_out == ''))
						{
							$attendance_body .= '<a href="'.base_url() . 'attendance/update_attendance/'.$user_id.'/'.$day.'/'.$attendance_month.'/'.$attendance_year.'" data-bs-toggle="ajaxModal" ><i class="fa fa-close text-danger" data-bs-toggle="tooltip" title="Check In not Yet"></i></a>';
						}
