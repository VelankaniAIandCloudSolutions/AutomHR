...
class Tickets extends MX_Controller {
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run() == FALSE)
			{
				...
				if($ticket_id = Ticket::save_data('tickets',$insert)){
					...
					$this->session->set_flashdata('tokbox_success', lang('ticket_created_successfully'));
	...
	function _admin_tickets($archive = FALSE,$filter_by = NULL){
		...
		if($filter_by == NULL) return Ticket::by_where(array('archived_t !=' => '1','branch_id'=>$this->session->userdata('branch_id')));

		if ($archive) return Ticket::by_where(array('archived_t' => '1','branch_id'=>$this->session->userdata('branch_id')));

		switch ($filter_by) {
			case 'open':
			return Ticket::by_where(array('archived_t !='=>'1','status' => 'open','branch_id'=>$this->session->userdata('branch_id')));
			break;
			case 'closed':
			return Ticket::by_where(array('archived_t !='=>'1','status' => 'closed','branch_id'=>$this->session->userdata('branch_id')));
			break;
			case 'pending':
			return Ticket::by_where(array('archived_t !='=>'1','status' => 'pending','branch_id'=>$this->session->userdata('branch_id')));
			break;
			case 'resolved':
			return Ticket::by_where(array('archived_t !='=>'1','status' => 'resolved','branch_id'=>$this->session->userdata('branch_id')));
			break;

			default:
			return Ticket::by_where(array('archived_t !='=>'1','branch_id'=>$this->session->userdata('branch_id')));
			break;
		}
	...
	function _staff_tickets($archive = FALSE, $filter_by = NULL){
		...
		if($filter_by == NULL){
			...
			($archive) ? $this->db->where(array('archived_t' => '1','branch_id'=>$this->session->userdata('branch_id')))
					: $this->db->where(array('archived_t !=' => '1','branch_id'=>$this->session->userdata('branch_id')));
		...
		switch ($filter_by) {
			case 'open':
			$this->db->where(array('archived_t !=' => '1','status' => 'open','branch_id'=>$this->session->userdata('branch_id')));
			...
			$this->db->where(array('archived_t !=' => '1','status' => 'closed','branch_id'=>$this->session->userdata('branch_id')));
			if(is_array($dep)){ $this->db->where_in('department', $dep); }else{
				$this->db->where('department',$staff_department);
			}
			return $this->db->or_where('reporter',User::get_id())->get('tickets')->result();
			...
			break;
			case 'pending':
			$this->db->where(array('archived_t !=' => '1','status' => 'pending','branch_id'=>$this->session->userdata('branch_id')));
			if(is_array($dep)){ $this->db->where_in('department', $dep); }else{
				$this->db->where('department',$staff_department);
			}
			return $this->db->or_where('reporter',User::get_id())->get('tickets')->result();
			...
			break;
			case 'resolved':
			$this->db->where(array('archived_t !=' => '1','status' => 'resolved','branch_id'=>$this->session->userdata('branch_id')));
			if(is_array($dep)){ $this->db->where_in('department', $dep); }else{
				$this->db->where('department',$staff_department);
			}
			return $this->db->or_where('reporter',User::get_id())->get('tickets')->result();
