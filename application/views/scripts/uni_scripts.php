        ...
        "aoColumnDefs":[{
          ...
          /*   var tableusers = $("#table-users").DataTable();
            ...
            $('#table-users_filter').hide();*/
             ...
             $("#table-branches-admin").DataTable();
...
function edit_model(task_id){
   
     $.post(base_url + 'all_tasks/get_task_details/', { task_id: task_id}, function (datas) {
         ...
         var data = JSON.parse(datas);        
               ...
               $("#progress_task"+task_id).val(data.task_progress);
               // $("#due_date").val(data.due_date);
               // $("#assigned_to_edit").val(data.assigned_to); 

               var progress = data.task_progress;

                $('#progress-slider_task'+task_id).noUiSlider({
            start: [ progress ],
            step: 10,
            connect: "lower",
            range: {
    ...
    $('#progress-slider_task'+task_id).on('slide', function() {
        var progress = $(this).val();
        $('#progress_task'+task_id).val(progress);
        $('.noUi-handle').attr('title', progress+'%').tooltip('fixTitle').parent().find('.tooltip-inner').text(progress+'%');
    });

    $('#progress-slider_task'+task_id).on('change', function() {
        var progress = $(this).val();
        $('#progress_task'+task_id).val(progress);
    });

    $('#progress-slider_task'+task_id).on('mouseover', function() {
        var progress = $(this).val();
        $('.noUi-handle').attr('title', progress+'%').tooltip('fixTitle').tooltip('show');
    });
...
$projects = $this->db->join('assign_projects','project_assigned = project_id')->where('projects.branch_id',$this->session->userdata('branch_id'))->where('assigned_user',User::get_id())->get('projects')->result();
...
$invoices = $this->db->where('invoices.branch_id',$this->session->userdata('branch_id'))->join('companies','client = co_id')->get('invoices')->result();
$estimates = $this->db->join('companies','client = co_id')->get('estimates')->result();
$projects = $this->db->where('projects.branch_id',$this->session->userdata('branch_id'))->join('companies','client = co_id')->get('projects')->result();
$events = $this->db->get_where('events',array('branch_id'=>$this->session->userdata('branch_id')))->result();
...
function compen_leave_day_type(){
  var val  = $("input[name='req_compen_leave_day_type']:checked").val();
  if(val == 2 || val == 3){  val = 0.5;  }
  $('#req_compen_leave_count').val(val);
}
...
function leave_dates(a){
	var dates = a.split("_");
	$('#req_compen_leave_date_from1').val(dates[0]);
	$('#req_compen_leave_date_to1').val(dates[0]);
	$('#req_compen_leave_count').val(dates[1]);
}
function compen_leave_days_calc(){
    $('#compen_leave_day_type').hide();

    var new_cnt = 0;

    if($('#req_compen_leave_date_from').val()!=''){
        var d1     = $('#req_compen_leave_date_from').datepicker('getDate');
		var d2     = $('#req_compen_leave_date_to').datepicker('getDate');
		if(d1  != null && d2 != null){
			new_cnt++;
			while(d1 <= d2){
			   if(d1.getDay() != 0){  new_cnt++; }
	...
	if(new_cnt == 1){
		$("input[name=req_compen_leave_day_type][value='1']").prop("checked",true);
		$('#compen_leave_day_type').show();
 	}
    if(new_cnt > 18){
        $('#lop_call').css('display','');
    }
    $('#req_compen_leave_count').val(new_cnt);
...
$(document).ready(function(){
	$("#emp_doj").datepicker({ });
	$("#req_compen_leave_date_to").datepicker({ });
	$("#req_compen_leave_date_from").datepicker({ });
	$("#dob").datepicker({ });
	$("#emp_gjd").datepicker({ });
...
console.log('asfsdgfdgh');
         ...
         $('#file_upload').on('change',function(e){
			   e.preventDefault();
				...
				$.ajax({
					url: '<?php echo base_url();?>all_tasks/post_task_comments/'+task+'/'+project,
         ...
         $('._file_updata').on('change',function(e){
			...
			$('._comment_upload').on('click',function(){
				...
				var data_id =$(this).data('taskid');
					...
					var comment_urls='<?php echo base_url();?>all_tasks/post_task_comments';   
    ...
    function call_comments(data)
    {
		...
		var comment_urls='<?php echo base_url();?>all_tasks/ajax_comments_update';   
		$.ajax({
				type : 'POST',
				url : comment_urls,
...
{
    var description =$('#task_description'+task_id).val();
    if(description!='')
    {
...
function task_name(task_id,project_id)
{

    var task_name =$('#task_name'+task_id).val();
    if(task_name!='')
    {
        $.post('<?php echo base_url();?>all_tasks/task_name_update',{'task_name':task_name,'task_id':task_id,'project_id':project_id},function(data){
            if(data == "false"){
                alert("This task name is already exist");
                return false
            }
...
function task_progress(task_id)
{
    var progress =$('#progress_task'+task_id).val();
   
    if(progress !='')
    {
        $.post('<?php echo base_url();?>all_tasks/progress_update',{'progress':progress,'task_id':task_id},function(data){
...
<script type="text/javascript">
    $(document).ready(function (e){
        $("#add_resignations").on('submit',(function(e){
			...
			if(add_branch == '')
            {
                toastr.error('Entity Field is Required');
                return false;
            }
             if(employee_id == '')
            {
...
function kanban_redirect(task_id,project_id,status){    
    window.location.href = base_url+'all_tasks/kanban_task_view/'+project_id+'/'+task_id+'/'+status ;
}
</script>
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("change", "#branchreportclient", function(){
            var branch_id = $(this).val();
            // alert();
            $.post(base_url + 'reports/choose_clients', {branch_id:branch_id}, function (data) {
                 var all_companies = JSON.parse(data);
                  var html = '';
                  if(all_companies.length > 0){
                // if(!data){
                    // console.log(data);
                    html += '<option value="" selected disabled>Choose Company</option>';
                    $( all_companies ).each(function( key,val ) {
                        
                        html += '<option value="'+val.co_id+'">'+val.company_name+'</option>';
                    });
                    $('#reportclient').html(html);
                }else{
                    $('#reportclient').html(html);
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("change", "#branchreportemployee", function(){
            var branch_id = $(this).val();
            alert(branch_id);
            $.post(base_url + 'reports/choose_projects', {branch_id:branch_id}, function (data) {
                 var all_projects = JSON.parse(data);
                  var html = '';
                  if(all_projects.length > 0){
                // if(!data){
                    console.log(data);
                    html += '<option value="" selected disabled>Choose Project</option>';
                    $( all_projects ).each(function( key,val ) {
                        
                        html += '<option value="'+val.project_id+'">'+val.project_title+'</option>';
                    });
                    $('#reportprojectlist').html(html);
                }else{
                    $('#reportprojectlist').html(html);
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("change", "#branchreporttask", function(){
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_tasks', {branch_id:branch_id}, function (data) {
                 var all_projects = JSON.parse(data);
                  var html = '';
                  if(all_projects.length > 0){
                // if(!data){
                    console.log(data);
                    html += '<option value="" selected disabled>Choose Task</option>';
                    $( all_projects ).each(function( key,val ) {
                        
                        html += '<option value="'+val.t_id+'">'+val.task_name+'</option>';
                    });
                    $('#tasklists').html(html);
                }else{
                    $('#tasklists').html(html);
                    toastr.error("No Tasks Found");
                    return false;
                }
            });
        });
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("change", "#branchreportemp", function(){
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_emp', {branch_id:branch_id}, function (data) {
                 var all_data = JSON.parse(data);
                  var employee = '';
                  var depart = '';
                    // console.log(all_data.employees); return false;
                  if(all_data.employees.length > 0){
                // if(!data){
                    employee += '<option value="" selected disabled>Choose Employees</option>';
                    $( all_data.employees ).each(function( key,val ) {
                        
                        employee += '<option value="'+val.user_id+'">'+val.fullname+'</option>';
                    });
                    $('#all_userids').html(employee);
                }else{
                    $('#all_userids').html(employee);
                    toastr.error("No Employees Found");
                    return false;
                }
                if(all_data.departments.length > 0){
                    depart += '<option value="" selected disabled>Choose Department</option>';
                    $( all_data.departments ).each(function( key,val ) {
                        
                        depart += '<option value="'+val.deptid+'">'+val.deptname+'</option>';
                    });
                    $('#all_departmentids').html(depart);
                }else{
                    $('#all_departmentids').html(depart);
                    toastr.error("No Departments Found");
                    return false;
                }
            });
        });
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("change", "#all_departmentids", function(){
            var depart_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_depart', {department_id:depart_id}, function (data) {
                 var all_departments = JSON.parse(data);
                 var html = '';
                  if(all_departments.length > 0){
                // if(!data){
                    // console.log(data);
                    html += '<option value="" selected disabled>Choose Designation</option>';
                    $( all_departments ).each(function( key,val ) {
                        
                        html += '<option value="'+val.id+'">'+val.designation+'</option>';
                    });
                    $('#all_designationids').html(html);
                }else{
                    $('#all_designationids').html(html);
                    toastr.error("No Designations Found");
                    return false;
                }
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("change", "#branchreportpayslip", function(){
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_payslipemp', {branch_id:branch_id}, function (data) {
                 var all_employees = JSON.parse(data);
                 var html = '';
                  if(all_employees.length > 0){
                // if(!data){
                    // console.log(data);
                    html += '<option value="" selected disabled>Choose Employee</option>';
                    $( all_employees ).each(function( key,val ) {
                        
                        html += '<option value="'+val.user_id+'">'+val.fullname+'</option>';
                    });
                    $('#all_payroleemployees').html(html);
                }else{
                    $('#all_payroleemployees').html(html);
                    toastr.error("No Employees Found");
                    return false;
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("change", "#branchreportattendance", function(){
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_attendance', {branch_id:branch_id}, function (data) {
                 var all_employees = JSON.parse(data);
                 var html = '';
                  if(all_employees.length > 0){
                // if(!data){
                    ...
                    $('#reportattendanceusers').html(html);
                }else{
                    $('#reportattendanceusers').html(html);
                    toastr.error("No Employees Found");
                    return false;
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("change", "#dailybranch_id", function(){
            var branch_id = $(this).val();
            // alert(branch_id);
            $.post(base_url + 'reports/choose_dailyemp', {branch_id:branch_id}, function (data) {
                 var all_employees = JSON.parse(data);
                 var html = '';
                  if(all_employees.length > 0){
                    html += '<option value="" selected disabled>Choose Employee</option>';
...
</script><?php if ($uri_page == 'reports') { ?>
<script>
function user_report_excel(name,table_id)
{
    var role_id   =  $('.role_id_excel').val();
    $.ajax({
        url: "<?=base_url()?>reports/user_report_excel/",
        type: 'POST',
        ...
        success: function(data) {
            ...
            var table = document.getElementById('user_report');//new line
            var innertable = table.innerHTML;
            //innertable = innertable.replace(/<\/?a>/g, '');
           ...
           // link.href = uri+template+ tab_text;
...
function employee_report_excel(name,table_id)
{
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            tab_text= tab_text.replace(/<img[^>]*>/gi,""); // remove if u want images in your table
            tab_text= tab_text.replace(/<input[^>]*>|<\/input>/gi, ""); 
            var table = document.getElementById('task_report');//new line
            var innertable = table.innerHTML;
            var ctx = {
                worksheet : name,
                table : innertable
...
{    
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var table = document.getElementById('task_report');//new line
            var innertable = table.innerHTML;
			if(name == 'Leave Report'){
				innertable = innertable.replaceAll('<td><div class="col-md-12 text-center"><div class="col-md-4 text-center">','<td><table style="width:100%;text-align:center"><tr><td>');
				innertable = innertable.replaceAll('</div><div class="col-md-4 text-center">','</td><td>');
				innertable = innertable.replaceAll('</div></div></td>','</td></tr></table></td>');
				innertable = innertable.replaceAll('<div class="col-md-4 text-center">','<td>');
				innertable = innertable.replaceAll('</div>','<td>');
			}
            var ctx = {
                worksheet : name,
                table : innertable
...
function export_report(report_id,name){
    var uri = 'data:application/vnd.ms-excel;base64,';
    var template = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table>{table}</table></body></html>'; 
    var base64 = function(s) {
        return window.btoa(unescape(encodeURIComponent(s)))
    ...
    var format = function(s, c) {
        ...
        // var tab_text="<table border='2px'><tr bgcolor='#1eb53a'>";
    var textRange; var j=0;
    var table = document.getElementById(report_id);//new line
    var innertable = table.innerHTML;
    var ctx = {
        worksheet : name,
        table : innertable
